...
query_TCGA = GDCquery(
  ...
  experimental.strategy = "RNA-Seq",
  workflow.type = "HTSeq - Counts",
  sample.type = c("Bone Marrow Normal", "Primary Blood Derived Cancer - Bone Marrow"))
...
limma_res = limma_pipeline(
  ...
  reference_group="Bone Marrow Normal"
)
```
...
png("gene_heatmap.png", width = 4, height = 4, units = 'in', res = 600)
# As you've seen a good looking heatmap involves a lot of parameters
gene_heatmap = heatmap.2(
  t(d_mat[,relevant_genes]),
  scale="row",          # scale the values for each gene (row)
...
clin_df = clinical[clinical$definition == "Primary Blood Derived Cancer - Bone Marrow",
                    c("patient",
                      "vital_status",
                      "days_to_death",
...
print(clin_df$overall_survival)
```
